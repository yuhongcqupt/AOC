...
if __name__ == '__main__':
    ...
    # names = ["housing-5bin", "LEV", "machine-5bin", "newthyroid", "stock-5bin", "SWD", "winequality-red"]
    # names = ["ERA","ESL","eucalyptus","housing-5bin","LEV","machine-5bin","newthyroid","stock-5bin","SWD","winequality-red"]
    # names = [ "ARWU2020-10bin"]
    names = ["housing-5bin"]

    # Methods = ["UP_median","UP_OR","TMUPOR","XTUPOR","HCAL_median","HCAL_OR","S2_median","S2_OR","MAED_median","MAED_OR","HS2MED","HS2OR"]
    # Methods = ["HCAL_median","HCAL_OR","S2_median","S2_OR","MAED_median","MAED_OR"]
    # Methods = ["MAED_median","MAED_OR"]
    # Methods = ["HS2MED", "HS2OR"]
            ...
            for train_idx, test_idx in SKF.split(X, y):
                ...
                for idxlist in label_dict.values():
                    for jdx in np.random.choice(idxlist, size=1, replace=False):
                        labeled.append(jdx)
                model = AOC_ECM(X_train=train_X, y_train=train_y, labeled=labeled, budget=Budget, X_test=test_X,
                               y_test=test_y)
                model.start()
