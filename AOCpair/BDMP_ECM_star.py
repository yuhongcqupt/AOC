...
class BDMP_ECM_star(object):
    ...
    def init_learning_model(self):
        ...
        for tar in self.target:
            model_dict[tar] = LogisticRegression(solver='liblinear')
        return model_dict
    ...
    def instance_selection(self):
        ...
        for idx in self.unLabeled:
            ...
            metric_list[idx] = max(prob.values())
            ...
            for train_idx, test_idx in SKF.split(X, y):
                ...
                for idxlist in label_dict.values():
                    for jdx in np.random.choice(idxlist, size=1, replace=False):
                        labeled.append(jdx)
                model = BDMP_ECM_star(X_train=train_X, y_train=train_y, labeled=labeled, budget=Budget, X_test=test_X,
                               y_test=test_y)
                model.start()
