...
class AOCmedian(object):
    ...
    ##################################################
    def median_label(self, idx):
        med = int(np.ceil(len(self.Xin[idx].inter) / 2))
        return self.Xin[idx].inter[med - 1]

    def contruct_med_pair(self):
        if self.intLabeled:
            for idx in self.intLabeled:
                self.Xin[idx].evalab = self.median_label(idx=idx)
            ...
            for train_idx, test_idx in SKF.split(X, y):
                ...
                for idxlist in label_dict.values():
                    for jdx in np.random.choice(idxlist, size=1, replace=False):
                        labeled.append(jdx)
                model = AOCmedian(X_train=train_X, y_train=train_y, labeled=labeled, budget=Budget, X_test=test_X,
                               y_test=test_y)
                model.start()
